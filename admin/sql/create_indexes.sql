...
CREATE UNIQUE INDEX token_ndx_token ON "token" (token);
CREATE UNIQUE INDEX token_api_key_ndx_token ON "token" (token, api_key);

CREATE UNIQUE INDEX sid_ndx_session ON "session" (sid);
CREATE UNIQUE INDEX sid_api_key_ndx_session ON "session" (sid, api_key);

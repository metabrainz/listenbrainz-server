...
class ListeningActivityTestCase(StatsTestCase):
    ...
    def test__get_time_range_bounds(self, mock_listen_ts):
        ...
        self.assertEqual((quarters[0], quarters[2], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("quarter"))
        mock_listen_ts.return_value = datetime(2021, 5, 7, 2, 3, 0)
        self.assertEqual((quarters[1], quarters[3], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("quarter"))
        mock_listen_ts.return_value = datetime(2021, 8, 9, 2, 3, 0)
        self.assertEqual((quarters[2], quarters[4], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("quarter"))
        mock_listen_ts.return_value = datetime(2021, 11, 8, 2, 3, 0)
        self.assertEqual((quarters[3], quarters[5], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("quarter"))
        ...
        self.assertEqual((periods[0], periods[2], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("half_yearly"))
        mock_listen_ts.return_value = datetime(2021, 9, 7, 2, 3, 0)
        self.assertEqual((periods[1], periods[3], step, fmt, spark_fmt), listening_activity_utils.get_time_range_bounds("half_yearly"))
        ...
        self.assertEqual(
            ...
            listening_activity_utils.get_time_range_bounds("this_week")
        )
        ...
        self.assertEqual(
            (datetime(2021, 11, 8), datetime(2021, 11, 22), step, fmt, spark_fmt),
            listening_activity_utils.get_time_range_bounds("week")
        )
        mock_listen_ts.return_value = datetime(2021, 11, 22, 3, 0, 0)
        self.assertEqual(
            (datetime(2021, 11, 8), datetime(2021, 11, 22), step, fmt, spark_fmt),
        ...
        self.assertEqual(
            ...
            listening_activity_utils.get_time_range_bounds("this_month")
        )
        ...
        self.assertEqual(
            (datetime(2021, 9, 1), datetime(2021, 11, 1), step, fmt, spark_fmt),
            listening_activity_utils.get_time_range_bounds("month")
        )
        mock_listen_ts.return_value = datetime(2021, 11, 1, 3, 0, 0)
        self.assertEqual(
            (datetime(2021, 9, 1), datetime(2021, 11, 1), step, fmt, spark_fmt),
        ...
        self.assertEqual(
            ...
            listening_activity_utils.get_time_range_bounds("year")
        )
        ...
        self.assertEqual(
            (datetime(2019, 1, 1), datetime(2021, 1, 1), step, fmt, spark_fmt),
            listening_activity_utils.get_time_range_bounds("this_year")
        )
        mock_listen_ts.return_value = datetime(2021, 11, 1, 3, 0, 0)
        self.assertEqual(

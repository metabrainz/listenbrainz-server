...
def calculate_daily_activity():
    ...
    formatted_listens = run_query("""
                            SELECT date_format(listened_at, 'EEEE') as day
...
def create_messages(data, stats_range: str, from_date: datetime, to_date: datetime) \
    ...
    message["data"] = _dict["daily_activity"]
    try:
        model = UserDailyActivityStatMessage(**message)
        result = model.dict(exclude_none=True)
        yield result
    except ValidationError:
        logger.error(f"""ValidationError while calculating {stats_range} daily_activity: 

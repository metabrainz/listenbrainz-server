...
def main():
    ...
    mlhd_df_path = config.HDFS_CLUSTER_URI + os.path.join('/data', 'mlhd', 'entire_mlhd.parquet')
    print('Loading MLHD Dataframe...')
    mlhd_df = listenbrainz_spark.sql_context.read.format('parquet').load(mlhd_df_path)
    print("Loaded!")
    ...
    print("Saving...")
    csv_path = config.HDFS_CLUSTER_URI + os.path.join('/data', 'mlhd', 'artist_popularity.csv')

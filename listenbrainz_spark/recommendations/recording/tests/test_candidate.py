...
class CandidateSetsTestClass(RecommendationsTestCase):
    ...
    @pytest.mark.xfail(reason="bug in candidate set generation, ambiguous recording_mbid")
    def test_get_top_artist_candidate_set(self):
        mapped_listens_df = utils.read_files_from_HDFS(RECOMMENDATION_RECORDING_MAPPED_LISTENS)
        recordings_df = create_dataframes.get_recordings_df(mapped_listens_df, {}, RECOMMENDATION_RECORDINGS_DATAFRAME)
        users = create_dataframes.get_users_dataframe(mapped_listens_df, {}, RECOMMENDATION_RECORDING_USERS_DATAFRAME)

        top_artist_limit = 1
        top_artist_df = candidate_sets.get_top_artists(mapped_listens_df, top_artist_limit, [])

        top_artist_candidate_set_df, top_artist_candidate_set_df_html = candidate_sets.get_top_artist_candidate_set(
            top_artist_df, recordings_df, users, mapped_listens_df
        )
    ...
    @pytest.mark.xfail(reason="need a sample artist relation dataset")
    def test_get_similar_artist_candidate_set_df(self):
        ...
        similar_artist_candidate_set_df, similar_artist_candidate_set_df_html = candidate_sets.get_similar_artist_candidate_set(
            similar_artist_df, recordings_df, users, mapped_listens_df
        )
    ...
    def test_filter_last_x_days_recordings(self):
        ...
        df = candidate_sets.filter_last_x_days_recordings(candidate_set_df, mapped_listens_df)

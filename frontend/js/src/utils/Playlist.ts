...
export async function enrichJSPFTracks(
    ...
    api: APIService,
  ): Promise<(JSPFObject["playlist"]["track"][number] & { 
    duration?: number; 
    extension?: { 
      [key: string]: { 
        additional_metadata: { 
      ...
      return {
        ...
        extension: {
          [MUSICBRAINZ_JSPF_TRACK_EXTENSION]: {
            ...
            added_by: track.extension?.[MUSICBRAINZ_JSPF_TRACK_EXTENSION]?.added_by || "",
            added_at: track.extension?.[MUSICBRAINZ_JSPF_TRACK_EXTENSION]?.added_at || "",
          },
        },
      };
    });

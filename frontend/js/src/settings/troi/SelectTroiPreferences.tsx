...
export default function SelectTroiPreferences() {
  const data = useLoaderData() as SelectTroiPreferencesLoaderData;
  const value = data?.troi_prefs?.troi?.export_to_spotify ?? false;

  const globalContext = React.useContext(GlobalAppContext);
  const { APIService, currentUser } = globalContext;
  const [exportToSpotify, setExportToSpotify] = React.useState(value);

  React.useEffect(() => {
    setExportToSpotify(value);
  }, [value]);
  ...
  return (
    <>
      ...
      <p>
        If auto-export is turned on, ListenBrainz will automatically export your
        generated playlists (Weekly Jams, Weekly Exploration, etc) to Spotify.
        <br />
        You can export playlists manually, regardless of whether auto-export is
        turned on or off.
      </p>

...
class DataTestCase(MessyBrainzTestCase):
    ...
    def test_get_id_from_meta_hash(self):
        recording_msid = data.submit_recording(self.msb_conn, recording)
        self.assertEqual(recording_msid, str(data.get_id_from_meta_hash(self.msb_conn, recording)))

    def test_submit_recording(self):
        recording_msid = data.submit_recording(self.msb_conn, recording)
        self.assertEqual(recording_msid, str(data.get_id_from_recording(self.msb_conn, recording)))

    def test_get_artist_credit(self):
        recording_msid = data.submit_recording(self.msb_conn, recording)
        artist_msid = data.get_artist_credit(self.msb_conn, recording['artist'])
        recording_data = data.load_recordings_from_msids(self.msb_conn, [recording_msid])[0]
        self.assertEqual(artist_msid, recording_data['ids']['artist_msid'])

    def test_get_release(self):
        recording_msid = data.submit_recording(self.msb_conn, recording)
        release_msid = data.get_release(self.msb_conn, recording['release'])
        recording_data = data.load_recordings_from_msids(self.msb_conn, [recording_msid])[0]
        self.assertEqual(release_msid, recording_data['ids']['release_msid'])

    def test_add_artist_credit(self):
        artist_msid = data.add_artist_credit(self.msb_conn, 'Kanye West')
        self.assertEqual(artist_msid, data.get_artist_credit(self.msb_conn, 'Kanye West'))

    def test_add_release(self):
        release_msid = data.add_release(self.msb_conn, 'The College Dropout')
        self.assertEqual(release_msid, data.get_release(self.msb_conn, 'The College Dropout'))

    def test_add_recording_different_cases(self):
        """ Tests that recordings with only case differences get the same MessyBrainz ID.
        """
        msid1 = data.submit_recording(self.msb_conn, recording)
        msid2 = str(data.get_id_from_recording(self.msb_conn, recording_diff_case))
        self.assertEqual(msid1, msid2)

    def test_load_recordings_from_msids(self):
        recording_msid = data.submit_recording(self.msb_conn, recording)
        result = data.load_recordings_from_msids(self.msb_conn, [recording_msid])[0]

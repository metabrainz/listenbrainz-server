...
def get_active_users_to_process(connection) -> List[dict]:
...
def get_user_import_details(connection, user_id: int) -> Optional[dict]:
...
def get_user(connection, user_id: int) -> Optional[dict]:

...
class UserSettingTestCase(DatabaseTestCase):
    def setUp(self):
        ...
        self.user = db_user.get_or_create(self.conn, 1, "user_setting_user")

    def test_set_timezone(self):
        # test if timezone is not null
        test_zonename = 'America/New_York'
        db_setting.set_timezone(self.conn, self.user['id'], test_zonename)
        result = db_setting.get(self.conn, self.user['id'])

...
class RecommendationFeedbackDatabaseTestCase(DatabaseTestCase):
    ...
    def setUp(self):
        ...
        self.user1 = db_user.get_or_create(self.conn, 2, "vansika_1")
        self.user2 = db_user.get_or_create(self.conn, 3, "vansika__2")
    ...
    def test_insert(self):
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user1['id'], limit=25, offset=0)
    ...
    def test_get_feedback_for_user(self):
        ...
        for i in range(60):
            ...
            db_feedback.insert(self.conn, submit_obj)
        ...
        # data
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=120, offset=0)
        self.assertEqual(len(result), 110)

        # test the rating argument
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=70, offset=0, rating='love')
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=70, offset=0, rating='hate')
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=20, offset=0, rating='love')
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=25, offset=10)
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=25, offset=100)
        ...
        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=30, offset=110)
        self.assertEqual(len(result), 0)

        result = db_feedback.get_feedback_for_user(self.conn, user_id=self.user2['id'], limit=30, offset=30, rating='hate')
        self.assertEqual(len(result), 20)
        for i in range(20):
    ...
    def test_get_feedback_count_for_user(self):
        self.insert_test_data()
        result = db_feedback.get_feedback_count_for_user(self.conn, user_id=self.user1["id"])

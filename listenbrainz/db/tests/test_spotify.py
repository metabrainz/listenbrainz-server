...
class SpotifyDatabaseTestCase(DatabaseTestCase):
    ...
    def test_get_active_users_to_process(self):
        user2 = db_user.get_or_create(self.conn, 2, 'newspotifyuser')
        db_oauth.save_token(
            self.conn,
            user_id=user2['id'],
            service=ExternalServiceType.SPOTIFY,
        ...
        db_import.update_import_status(self.conn, user2['id'], ExternalServiceType.SPOTIFY, 'something broke')
        db_import.update_import_status(self.conn, user3['id'], ExternalServiceType.SPOTIFY, 'oops.')
        users = db_spotify.get_active_users_to_process(self.conn)

...
class ListensImporterDatabaseTestCase(DatabaseTestCase):
    ...
    def test_update_import_status(self):
        db_import.update_import_status(self.conn, self.user['id'], ExternalServiceType.SPOTIFY, 'test error message')
        spotify_user = db_spotify.get_user_import_details(self.conn, self.user['id'])
        self.assertEqual(spotify_user['error_message'], 'test error message')
        self.assertIsNotNone(spotify_user['last_updated'])

        db_import.update_import_status(self.conn, self.user['id'], ExternalServiceType.SPOTIFY)
        spotify_user = db_spotify.get_user_import_details(self.conn, self.user['id'])
    ...
    def test_update_latest_listened_at(self):
        ...
        t = int(time.time())
        db_import.update_latest_listened_at(self.conn, self.user['id'], ExternalServiceType.SPOTIFY, t)
        spotify_user = db_spotify.get_user_import_details(self.conn, self.user['id'])
        self.assertEqual(t, int(spotify_user['latest_listened_at'].strftime('%s')))
        self.assertIsNotNone(spotify_user['last_updated'])

    def test_update_latest_import(self):
        user = db_user.get_or_create(self.conn, 3, 'updatelatestimportuser')

        val = int(time.time())
        listens_importer.update_latest_listened_at(self.conn, user['id'], ExternalServiceType.LASTFM, val)
        ts = listens_importer.get_latest_listened_at(self.conn, user['id'], ExternalServiceType.LASTFM)
        self.assertEqual(int(ts.strftime('%s')), val)

        listens_importer.update_latest_listened_at(self.conn, user['id'], ExternalServiceType.LASTFM, 0)
        ts = listens_importer.get_latest_listened_at(self.conn, user['id'], ExternalServiceType.LASTFM)

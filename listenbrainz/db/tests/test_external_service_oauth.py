...
class OAuthDatabaseTestCase(DatabaseTestCase):
    ...
    def test_create_oauth(self):
        user2 = db_user.get_or_create(self.conn, 2, 'spotify')
        db_oauth.save_token(
            self.conn,
            user_id=user2['id'],
            service=ExternalServiceType.SPOTIFY,
        ...
        user = db_oauth.get_token(self.conn, user2['id'], ExternalServiceType.SPOTIFY)
    ...
    def test_create_oauth_multiple(self):
        ...
        user = db_oauth.get_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY)
    ...
    def test_update_token(self):
        ...
        spotify_user = db_oauth.get_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY)
    ...
    def test_delete_token_unlink(self):
        db_oauth.delete_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY, remove_import_log=True)
        self.assertIsNone(db_oauth.get_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY))
        self.assertIsNone(db_spotify.get_user_import_details(self.conn, self.user['id']))

    def test_delete_token_retain_error(self):
        db_oauth.delete_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY, remove_import_log=False)
        self.assertIsNone(db_oauth.get_token(self.conn, self.user['id'], ExternalServiceType.SPOTIFY))
        self.assertIsNotNone(db_spotify.get_user_import_details(self.conn, self.user['id']))

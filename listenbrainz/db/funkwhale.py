...
def get_or_create_server(db_conn: Connection, host_url: str, client_id: str, client_secret: str, scopes: str) -> int:
...
def get_token(db_conn: Connection, user_id: int, funkwhale_server_id: int) -> Optional[Dict[str, Any]]:
    result = db_conn.execute(sqlalchemy.text("""
...
def save_token(db_conn: Connection, user_id: int, funkwhale_server_id: int, access_token: str, refresh_token: str, token_expiry) -> int:
    result = db_conn.execute(sqlalchemy.text("""
...
def update_token(db_conn: Connection, user_id: int, funkwhale_server_id: int, access_token: str, refresh_token: str, token_expiry) -> None:
...
def delete_token(db_conn: Connection, user_id: int, funkwhale_server_id: int) -> None:
    db_conn.execute(sqlalchemy.text("""
        DELETE FROM funkwhale_tokens WHERE user_id = :user_id AND funkwhale_server_id = :funkwhale_server_id
    """), {'user_id': user_id, 'funkwhale_server_id': funkwhale_server_id})
    db_conn.commit()

def get_all_user_tokens(db_conn: Connection, user_id: int) -> list:

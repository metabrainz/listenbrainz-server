...
def bulk_insert(slug, playlists):
    """ Bulk insert playlists generated by spark """
    user_ids = [p["user_id"] for p in playlists]
    user_names = get_users_by_id(user_ids)
    ...
    jam_date = "week of " + datetime.datetime.now().strftime("%Y-%m-%d %a")
    ...
    try:
        with conn.cursor() as curs:
            results = execute_values(curs, playlist_query, users, playlist_template, fetch=True)

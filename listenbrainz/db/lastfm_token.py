...
class Token(object):
    ...
    def __init__(self, connection, id, userid, token, api_key, ts):
        self.id = id
        self.token = token
        self.api_key = api_key
        self.timestamp = ts
        self.user = User.load_by_id(connection, userid)
    ...
    @staticmethod
    def load(connection, token, api_key=None):
    ...
    @staticmethod
    def generate(connection, api_key):
        ...
        return Token.load(connection, token)
    ...
    def approve(self, connection, user):
        ...
        connection.execute(text("UPDATE api_compat.token SET user_id = :uid WHERE token=:token"),
                           {'uid': User.get_id(connection, user), 'token': self.token})
        self.user = User.load_by_name(connection, user)

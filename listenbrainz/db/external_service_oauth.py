...
def get_import_data(db_conn, user_id: int, service: ExternalServiceType) -> [dict]:
    ...
    """
    result = db_conn.execute(sqlalchemy.text("""
        SELECT import_status, imported_listens
        FROM external_service_oauth
        WHERE user_id = :user_id 
        AND service = :service
...
def update_import_data(db_conn, user_id: int, service: ExternalServiceType, import_status: str, imported_listens: int):
    """ 

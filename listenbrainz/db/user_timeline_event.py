...
def create_user_track_recommendation_event(
    connection,
    user_id: int,
...
def create_user_cb_review_event(
    connection,
    user_id: int,
...
def get_user_track_recommendation_events(
    connection,
    user_id: int,
    count: int = 50
) -> List[UserTimelineEvent]:
...
def get_user_timeline_event_by_id(connection, id: int) -> UserTimelineEvent:
...
def get_user_notification_events(connection, user_id: int, count: int = 50) -> List[UserTimelineEvent]:
...
def get_hidden_timeline_events(connection, user_id: int, count: int) -> List[HiddenUserTimelineEvent]:
...
def unhide_timeline_event(connection, user: int, event_type: UserTimelineEventType, event_id: int) -> bool:
    ''' Deletes hidden timeline events for a user with specific row id '''
    result = connection.execute(sqlalchemy.text('''
        DELETE FROM hide_user_timeline_event
              WHERE user_id = :user_id
                AND event_type = :event_type

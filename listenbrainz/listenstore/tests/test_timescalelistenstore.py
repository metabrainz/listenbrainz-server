...
class TestTimescaleListenStore(DatabaseTestCase, TimescaleTestCase):
    ...
    def _insert_mapping_metadata(self, msid):
        ...
        self.ts_conn.execute(sqlalchemy.text(join_query))

    def test_insert_timescale(self):
        count = self._create_test_data(self.testuser_name, self.testuser_id)
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1399999999)
    ...
    def test_fetch_listens_0(self):
        ...
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1400000000,
                                                              limit=1)
    ...
    def test_fetch_listens_2(self):
        self._create_test_data(self.testuser_name, self.testuser_id)
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1400000100)
    ...
    def test_fetch_listens_4(self):
        self._create_test_data(self.testuser_name, self.testuser_id)
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1400000049,
                                                              to_ts=1400000101)
        self.assertEqual(len(listens), 2)
    ...
    def test_fetch_listens_5(self):
        ...
        with self.assertRaises(ValueError):
            self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1400000101, to_ts=1400000001)
    ...
    def test_fetch_listens_with_gaps(self):
        ...
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, to_ts=1420000051)
        self.assertEqual(len(listens), 4)
        self.assertEqual(listens[0].ts_since_epoch, 1420000050)
        self.assertEqual(listens[1].ts_since_epoch, 1420000000)
        self.assertEqual(listens[2].ts_since_epoch, 1400000050)
    ...
    def test_get_listen_count_for_user(self):
        ...
        testuser = db_user.get_or_create(self.conn, uid, "user_%d" % uid)
        testuser_name = testuser['musicbrainz_id']

        count = self._create_test_data(testuser_name, testuser["id"])
        listen_count = self.logstore.get_listen_count_for_user(self.ts_conn, testuser["id"])
        self.assertEqual(count, listen_count)

    def test_fetch_recent_listens(self):
        user = db_user.get_or_create(self.conn, 2, 'someuser')
        user_name = user['musicbrainz_id']
        self._create_test_data(user_name, user["id"])

        user2 = db_user.get_or_create(self.conn, 3, 'otheruser')
        user_name2 = user2['musicbrainz_id']
        self._create_test_data(user_name2, user2["id"])

        recent = self.logstore.fetch_recent_listens_for_users(self.ts_conn, [user, user2], per_user_limit=1,
                                                              min_ts=int(time()) - 10000000000)
        self.assertEqual(len(recent), 2)

        recent = self.logstore.fetch_recent_listens_for_users(self.ts_conn, [user, user2],
                                                              min_ts=int(time()) - 10000000000)
        self.assertEqual(len(recent), 4)

        recent = self.logstore.fetch_recent_listens_for_users(self.ts_conn, [user], min_ts=recent[0].ts_since_epoch - 1)
    ...
    def test_listen_counts_in_cache(self):
        ...
        self.assertEqual(count, self.logstore.get_listen_count_for_user(self.ts_conn, testuser["id"]))
    ...
    def test_delete_listens(self):
        ...
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=testuser, to_ts=1400000300)
        self.assertEqual(len(listens), 5)
        self.assertEqual(listens[0].ts_since_epoch, 1400000200)
        self.assertEqual(listens[1].ts_since_epoch, 1400000150)
        self.assertEqual(listens[2].ts_since_epoch, 1400000100)
        ...
        self.logstore.delete(self.ts_conn, testuser["id"])
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=testuser, to_ts=1400000300)
    ...
    def test_delete_single_listen(self):
        ...
        self.logstore.delete_listen(self.ts_conn, 1400000050, testuser["id"], "c7a41965-9f1e-456c-8b1d-27c0f0dde280")
        ...
        self.assertEqual(self.logstore.get_listen_count_for_user(self.ts_conn, testuser["id"]), 4)
        min_ts, max_ts = self.logstore.get_timestamps_for_user(self.ts_conn, testuser["id"])
        self.assertEqual(min_ts, 1400000000)
        self.assertEqual(max_ts, 1400000200)

    def _get_pending_deletes(self):
        result = self.ts_conn.execute(text("SELECT * FROM listen_delete_metadata"))
    ...
    def test_for_empty_timestamps(self):
        ...
        uid = random.randint(2000, 1 << 31)
        testuser = db_user.get_or_create(self.conn, uid, "user_%d" % uid)
        self.logstore.set_empty_values_for_user(self.ts_conn, testuser["id"])
    ...
    def test_get_total_listen_count(self):
        ...
        uid = random.randint(2000, 1 << 31)
        testuser2 = db_user.get_or_create(self.conn, uid, f"user_{uid}")
    ...
    def test_get_timestamps_for_user(self):
        ...
        min_ts, max_ts = self.logstore.get_timestamps_for_user(self.ts_conn, self.testuser["id"])
    ...
    def test_fetch_listens_for_since_cron_run(self):
        ...
        listens, min_ts, max_ts = self.logstore.fetch_listens(self.ts_conn, user=self.testuser, from_ts=1400000300,
                                                              limit=1)
        self.assertEqual(len(listens), 1)

...
FakeEntry = namedtuple("FakeEntry", ["recording_mbid", "recording_data", "artist_data", "release_data", "tag_data"])

FAKE_ENTRY = FakeEntry(
    recording_mbid=RECORDING_MBID,
    recording_data={"name": "Creep", "length": 238000},
    artist_data={
        "name": "Radiohead",
    ...
    tag_data={"artist": [], "release_group": [], "recording": []},
)
...
class TestRecordingLookupService:
    ...
    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_lookup_returns_keyed_by_mbid(self, mock_db):
        svc = RecordingLookupService()
        result = svc.lookup([RECORDING_MBID], "artist release")
        assert RECORDING_MBID in result

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_lookup_includes_artist_when_in_inc(self, mock_db):
        result = RecordingLookupService().lookup([RECORDING_MBID], "artist release")
        assert result[RECORDING_MBID]["artist"]["name"] == "Radiohead"

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_lookup_includes_release_when_in_inc(self, mock_db):
        result = RecordingLookupService().lookup([RECORDING_MBID], "artist release")
        assert result[RECORDING_MBID]["release"]["name"] == "Pablo Honey"

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_lookup_excludes_tag_when_not_in_inc(self, mock_db):
        result = RecordingLookupService().lookup([RECORDING_MBID], "artist release")
        assert "tag" not in result[RECORDING_MBID]

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_lookup_includes_tag_when_in_inc(self, mock_db):
        result = RecordingLookupService().lookup([RECORDING_MBID], "artist release tag")
        assert "tag" in result[RECORDING_MBID]

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[])
    def test_lookup_empty_returns_empty_dict(self, mock_db):
        result = RecordingLookupService().lookup([RECORDING_MBID], "artist release")
        assert result == {}

    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_troi_uses_service_instead_of_http(self, mock_db):
    ...
    @patch("listenbrainz.radio.recording_lookup.get_metadata_for_recording", return_value=[FAKE_ENTRY])
    def test_troi_uses_service_sets_recording_name(self, mock_db):
        element = RecordingLookupElement()
        mock_patch = MagicMock()
        mock_patch.services = {"recording-lookup": RecordingLookupService()}
        element.patch = mock_patch

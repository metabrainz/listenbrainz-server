    ...
    try:
        with mb_conn.cursor() as curs:
            curs.execute("""CREATE INDEX top_discoveries_2021_ndx_user_name
                                      ON top_discoveries_2021 (user_name)"""
            mb_conn.commit()
    except (psycopg2.errors.OperationalError, psycopg2.errors.UndefinedTable) as err:
    ...
    with lb_conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as lb_curs:
        ...
        query = """SELECT user_name
          ...
          FULL OUTER JOIN listen_mbid_mapping m
                 ...
                 GROUP BY user_name, recording_mbid, artist_name, track_name
        ...
        while True:
            ...
            if not row:
                ...
                if len(saved_row["years"]) > 1:
                    top_recordings.append((saved_row["rec_msid"],
                                           saved_row["recording_mbid"],
                                           saved_row["track_name"],
                                           saved_row["artist_name"],
                                           len(saved_row["years"]),
                                           saved_row["user_name"]))
...
def get_top_discoveries(year):
    ...
    with psycopg2.connect(config.TIMESCALE_DATABASE_URI) as lb_conn:
        with psycopg2.connect(config.SQLALCHEMY_DATABASE_URI) as mb_conn:
            create_table(mb_conn)
            fetch_top_discoveries_for_users(lb_conn, mb_conn, ["rob", "mr_monkey"]):

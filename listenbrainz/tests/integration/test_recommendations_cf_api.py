...
class CFRecommendationsViewsTestCase(IntegrationTestCase):
    def setUp(self):
        ...
        db_recommendations_cf_recording.insert_user_recommendation(1, ["526bd613-fddd-4bd6-9137-ab709ac74cab"], [])
        db_recommendations_cf_recording.insert_user_recommendation(2, [], ["526bd613-fddd-4bd6-9137-ab709ac74cab"])
    ...
    def test_top_artist_recommendations_not_generated(self):
        response = self.client.get(url_for('recommendations_cf_recording_v1.get_recommendations',
                                           user_name=self.user2['musicbrainz_id']), query_string={'type': 'top'})
        self.assertEqual(response.status_code, 204)

    def test_similar_artist_recommendations_not_generated(self):
        response = self.client.get(url_for('recommendations_cf_recording_v1.get_recommendations',
                                           user_name=self.user['musicbrainz_id']), query_string={'type': 'similar'})
        self.assertEqual(response.status_code, 204)

    def test_top_artist_recommendations(self):
        response = self.client.get(url_for('recommendations_cf_recording_v1.get_recommendations',
        ...
        expected_top_artist_recommendations = self.user_recommendations['recording_mbid']['top_artist']
        self.assertEqual(expected_top_artist_recommendations, received_top_artist_recommendations)

    def test_similar_artist_recommendations(self):
        response = self.client.get(url_for('recommendations_cf_recording_v1.get_recommendations',
                                           user_name=self.user2['musicbrainz_id']), query_string={'type': 'similar'})
        self.assert200(response)
        data = json.loads(response.data)['payload']
        ...
        expected_top_artist_recommendations = self.user2_recommendations['recording_mbid']['similar_artist']
        self.assertEqual(expected_top_artist_recommendations, received_top_artist_recommendations)

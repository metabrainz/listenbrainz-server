...
describe("handleClickNewerRecommendations", () => {
  it("don't do anything if already on newest/first page i.e. offset === 0", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();

    const recs = recentListensProps.listens.slice(0, 25);
    wrapper.setState({ offset: 0, listens: recs });

    await instance.handleClickNewerRecommendations();

    expect(wrapper.state("offset")).toEqual(0);
    expect(wrapper.state("listens")).toEqual(recs);
  });

  it("go to the previous page and update listens if offset !== 0", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();

    wrapper.setState({
      offset: 50,
      listens: recentListensProps.listens.slice(50, 53),
    });

    await instance.handleClickNewerRecommendations();

    expect(wrapper.state("offset")).toEqual(25);
    expect(wrapper.state("listens").length).toEqual(25);
  });
});

describe("handleClickNewestRecommendations", () => {
  it("don't do anything if already on newest/first page i.e. offset === 0", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();
    ...
    await instance.handleClickNewestRecommendations();

    expect(wrapper.state("offset")).toEqual(0);
    expect(wrapper.state("listens")).toEqual(recs);
  });

  it("move to the newest/first page if offset !== 0", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();
...
describe("handleClickOldestRecommendations", () => {
  it("don't do anything if already on the last page", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();

    const recs = recentListensProps.listens.slice(50, 53);
    wrapper.setState({ offset: 50, listens: recs, recommendationLength: 52 });

    await instance.handleClickOldestRecommendations();

    expect(wrapper.state("offset")).toEqual(50);
    expect(wrapper.state("listens")).toEqual(recs);
  });
  /*
  it("move to the last page", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();

    wrapper.setProps({listens: recentListensProps.listens});
    wrapper.setState({ offset: 25, listens: recentListensProps.listens.slice(25, 50), recommendationLength: 52});
...
/*
describe("handleClickOlderRecommendations", () => {
  it("don't do anything if already on the last page", async () => {
    const wrapper = shallow<RecentListens>(<RecentListens {...props} />);
    const instance = wrapper.instance();
    ...
    await instance.handleClickOlderRecommendations();
  ...
  it("move to the next page", async () => {
    ...
    wrapper.setState({ offset: 0, listens: recentListensProps.listens.slice(0, 25), recommendationLength: 52});

...
class RecentListens extends React.Component {
  ...
  constructor(props) {
    ...
    this.getRecentListensForFollowList = this.getRecentListensForFollowList.bind(
      this
    );
    this.handleCurrentListenChange = this.handleCurrentListenChange.bind(this);
    this.handleFollowUserListChange = this.handleFollowUserListChange.bind(
      this
    );
    this.handleSpotifyAccountError = this.handleSpotifyAccountError.bind(this);
    this.handleSpotifyPermissionError = this.handleSpotifyPermissionError.bind(
      this
    );
  ...
  handleFollowUserListChange(userList, dontSendUpdate) {
    if (!Array.isArray(userList)) {
      console.error(
        "Expected array in handleFollowUserListChange, got",
        typeof userList
      );
      return;
    }
    ...
    this.setState(
      ...
      () => {
        ...
        if (
          ...
          _.difference(userList, previousFollowList)
        ) {
          this.getRecentListensForFollowList();
        }
      }
  ...
  isCurrentListen(listen) {
    return (
      this.state.currentListen && _isEqual(listen, this.state.currentListen)
    );
  }
  ...
  getRecentListensForFollowList() {
    ...
    this.APIService.getRecentListensForUsers(this.state.followList)
      .then((listens) =>
        this.setState({ listens: _.orderBy(listens, "listened_at", "asc") })
      )
      .catch((error) =>
        this.newAlert("danger", "Could not get recent listens", error)
      );
  }
  ...
  render() {
    const getSpotifyEmbedSrc = () => {
      ...
      const spotifyListens = this.state.listens.filter(
        (listen) =>
          _.get(
            listen,
    ...
    return (
      <div>
        ...
        <div className="row">
          <div className="col-md-8">
            <h3>
              {this.state.mode === "listens" || this.state.mode === "recent"
                ? "Recent listens"
                : "Playlist"}
            </h3>
              ...
              <div>
                ...
                >
                  <thead>
                    <tr>
                      ...
                      <th>Time</th>
                      {(this.state.mode === "follow" ||
                        this.state.mode === "recent") && <th>User</th>}
                      <th width="50px" />
                    </tr>
                  </thead>
                  <tbody>
                    {this.state.listens
                      .sort((a, b) =>
                        a.playing_now ? -1 : b.playing_now ? 1 : 0
                      )
                      .map((listen, index) => {
                        return (
                          <tr
                          ...
                          >
                            ...
                            {listen.playing_now ? (
                              <td>
                                <FontAwesomeIcon icon={faMusic} /> Playing now
                              </td>
                            ) : (
                              <td>
                ...
                {this.state.mode === "listens" && (
                  <ul className="pager">
                    <li
                      className="previous"
                      className={!this.props.previous_listen_ts ? "hidden" : ""}
                    >
                      <a
                        href={`${this.props.profile_url}?min_ts=${this.props.previous_listen_ts}`}
                      >
                        &larr; Previous
                      </a>
                    </li>
                    <li
                      className="next"
                      disabled={!this.props.next_listen_ts ? "hidden" : ""}
                    >
                      <a
                        href={`${this.props.profile_url}?max_ts=${this.props.next_listen_ts}`}
                      >
                        Next &rarr;
                      </a>
                    </li>
            ...
            {this.state.mode === "follow" && (
              <FollowUsers
                ...
                playListen={this.playListen.bind(this)}

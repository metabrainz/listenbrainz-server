...
describe("deleteFeedback", () => {
  ...
  it("does nothing if isCurrentUser is false", async () => {
    const wrapper = shallow<RecommendationCard>(
      <RecommendationCard {...{ ...props, isCurrentUser: false, updateFeedback: jest.fn() }} />
    );
    const instance = wrapper.instance();
    const updateFeedbackSpy = jest.spyOn(instance.props, "updateFeedback");
  ...
  it("does nothing if CurrentUser.authtoken is not set", async () => {
    const wrapper = shallow<RecommendationCard>(
      <RecommendationCard
        {...{ ...props, currentUser: { auth_token: undefined, name: "test"}, updateFeedback: jest.fn() }}
      />
    );
    const instance = wrapper.instance();
    const updateFeedbackSpy = jest.spyOn(instance.props, "updateFeedback");
...
describe("Check if button and dropdown values ae synced.", () => {
  it("check button and dropdown values when currentFeedback == 'Hate' ", async () => {
    ...
    expect(myComponents.get(0).props.title).toEqual("I never want to hear this again!")
  ...
  it("check button and dropdown values when currentFeedback == 'dislike' ", async () => {
    ...
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(0).prop("icon").iconName).toEqual("frown");
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(2).text()).toEqual("Dislike");
  ...
  it("check button and dropdown values when currentFeedback == 'bad_recommendation' ", async () => {
    const wrapper = shallow<RecommendationCard>(
      <RecommendationCard {...{ ...props, currentFeedback: 'bad_recommendation' }} />
    );
    const myComponents = wrapper.find(RecommendationControl);

    // validate button class and properties
    expect(
      wrapper.find(".recommendation-controls").childAt(0).hasClass("bad_recommendation")
    ).toEqual(true);
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(0).prop("icon").iconName).toEqual("meh");
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(2).text()).toEqual("Bad");

    // validate RecommendationComponent props for 'Meh' emoticon
    expect(myComponents.get(2).props.iconHover).toEqual(faMeh);
    expect(myComponents.get(2).props.icon).toEqual(faMehRegular);
    expect(myComponents.get(2).props.cssClass).toEqual('bad_recommendation selected');
    expect(myComponents.get(2).props.title).toEqual("This is a bad recommendation!");
  ...
  it("check button and dropdown values when currentFeedback == 'like' ", async () => {
    ...
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(0).prop("icon").iconName).toEqual("smile-beam");
    expect(wrapper.find('.recommendation-controls').childAt(0).childAt(2).text()).toEqual("Like");

...
class ProfileViewsTestCase(IntegrationTestCase):
    ...
    def test_reset_import_timestamp(self):
        with self.app.app_context():
            ...
            val = int(time.time())
            listens_importer.update_latest_listened_at(self.conn, self.user['id'], ExternalServiceType.LASTFM, val)
            self.temporary_login(self.user['login_id'])
            ...
            ts = listens_importer.get_latest_listened_at(self.conn, self.user['id'], ExternalServiceType.LASTFM)
    ...
    def test_delete_listens_csrf_token_not_provided(self):
        ...
        with self.app.app_context():
            ...
            response = self.client.post(delete_listens_url)
            self.assertMessageFlashed('Cannot delete listens due to error during authentication, please try'
                                      ' again later.', 'error')
    ...
    def test_select_timezone(self):
        ...
        with self.app.app_context():
            ...
            response = self.client.post(select_timezone_url,
                                        data={'csrf_token': g.csrf_token, 'timezone': 'America/New_York'})

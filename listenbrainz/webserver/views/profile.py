...
def select_timezone():
    ...
    if form.validate_on_submit():
        try:
            ...
            db_usersetting.set_timezone(db_conn, current_user.id, update_timezone)
...
@login_required
def info():
    user_setting = db_usersetting.get(db_conn, current_user.id)
    return render_template(
        "profile/info.html",
        user=current_user,
...
def _get_service_or_raise_404(name: str) -> ExternalService:
    ...
    try:
        ...
        if service == ExternalServiceType.YOUTUBE:
            ...
            return SpotifyService(db_conn)
        elif service == ExternalServiceType.CRITIQUEBRAINZ:
            return CritiqueBrainzService(db_conn)
    except KeyError:
...
def music_services_details():
    spotify_service = SpotifyService(db_conn)
